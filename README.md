...
3. **CIE76 Delta-E Calculation**:
   Calculates color difference ($\Delta E^*_{ab}$) between the target color ($L_1^*, a_1^*, b_1^*$) and candidate markers ($L_2^*, a_2^*, b_2^*$) using standard Euclidean distance in $L^*a^*b^*$ space:
